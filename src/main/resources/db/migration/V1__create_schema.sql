...
--   1. customer, company       — sem dependências
--   2. body_profile, product   — dependem de customer / company
--   3. wishlist, wardrobe_item, look — dependem de customer / product
--   4. wishlist_item, look_wardrobe_item, look_product — junções
--
-- Boas práticas aplicadas:
-- * uuid como PK — sem sequência exposta na URL
-- * timestamptz — armazena fuso horário
-- * password_hash — nunca texto puro (LGPD + segurança)
-- * UNIQUE(customer_id) em body_profile e wishlist → relação 1:1
-- * product_id nullable em wardrobe_item → peça pode ser fotografada
--   manualmente, sem vínculo com produto de loja
-- * Tabelas de junção com uuid próprio + UNIQUE INDEX nas combinações
